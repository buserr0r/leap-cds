...
function matchPurposeOfUse(provision, query) {
  const untrimmedPurposes = _.get(provision, "purpose") || [];
...
async function fetchActor(actorReference, fhirBase) {
  return superagent
    ...
    .set({ Accept: "application/json" , Authorization: global.TOKEN});
}
...
function determineObligations(query, decision, provision) {
  ...
  const allCodes = _.concat(
    ...
    contentClasses,
    clinicalCodes
  ).filter(entry => entry).map(code => _.pick(code, ["system", "code"]));
