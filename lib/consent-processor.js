...
async function processDecision(consentsBundle, query, fhirServer, fhirAuthorization) {
  ...
  const consentDecisionPromises = consentsBundle
    ...
    .filter(entry =>
      matchPurposeOfUse(_.get(entry, "resource.provision"), query)
    )
  ...
  await AuditService.maybeRecordAudit(finalDecision, query, fhirServer, fhirAuthorization);
...
function matchesCategory(entry, query) {
  const untrimmedCategories = _.get(entry, "resource.category") || [];
  const untrimmedQueriedCategories = query.category || [];

  const categories = _.flatten(
    untrimmedCategories.map(categoryCode => _.get(categoryCode, "coding"))
  )
    .map(category => _.pick(category, ["system", "code"]))
    .filter(category => category);

  const queriedCategories = untrimmedQueriedCategories.map(category =>
    _.pick(category, ["system", "code"])
  );
